...
public class Game {
     ...
     * @throws IOException
     */
    public static void generatePlayers(int n) throws IOException {
...
//         allPlayers.add(p5);
        ...
        int max = 2;
        if (n == -1) max = Integer.parseInt(reader.readLine());
        else max = n;
        for (int i = 0; i < max; i++) {
            if (i == 0) {
    ...
    public static void dashAttack() {
        ...
        if (field.get(0).getHp() > 0 && field.get(0).isDash()) {
            if (!field.get(0).getActiveMarkers().isEmpty()) {
                if (field.get(0).getActiveMarkers().contains(Marker.FREEZEMARKER))
...
//      Фаза 1: перемещание
        if (turnCount == 0) {
            players.get(0).moveDecision(field, "s");
            turnCount++;
        } else if (turnCount == 1 && players.size() == 2) {
            players.get(1).moveDecision(field, "s");
            turnCount--;
        }
//        else if(turnCount == 1 && players.size() > 2){
...
//        }
        ...
        for (Player player : players) {
            if (player.getTarget().getHp() < 0) {
                player.getTarget().setDead(player);
                System.out.println(player.getTarget() + " погиб и пропускает ход!");
            } else {
        ...
        for (Player player : players) {
            if (player.getTarget().getHp() < 0) {
                ...
                if (!player.getTarget().getActiveMarkers().isEmpty()) {
                    if (!player.getTarget().getActiveMarkers().getLast().equals(Marker.FREEZEMARKER)) {
                        if (player.drawDecision("s")) player.drawSpell(field, player, -1);
                    } else {
                        player.discardAll(field);
                        System.out.println(player + " заморожен и пропускает ход!");
                    }
